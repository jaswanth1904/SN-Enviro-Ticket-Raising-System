    ...
    <div key={index} className="bg-white rounded-2xl border border-gray-200 shadow-sm hover:shadow-md hover:border-blue-300 transition-all duration-300 overflow-hidden group">
      <div className="p-6">
        ...
        <div className="space-y-3 mt-4">
          ...
          <div className="flex items-center justify-between text-[13px] text-gray-700 bg-blue-50/50 p-3 rounded-xl border border-blue-100 group-hover:bg-blue-50 transition-colors mt-3">
            <div className="flex items-center break-words min-w-0 mr-3">
              <Mail className="w-4.5 h-4.5 text-blue-600 mr-3 shrink-0" />
              <span className="font-semibold text-blue-900 break-words">{engineer.email}</span>
            </div>
            <button 
              onClick={() => handleCopyEmail(engineer.email)}
              className="p-2 text-blue-600 hover:bg-blue-600 hover:text-white bg-blue-100 rounded-lg transition-colors shrink-0 shadow-sm"
  ...
  return (
    <div className="space-y-12 max-w-[1400px] mx-auto pb-16 px-4 sm:px-6">
      ...
      {embeddedEngineers.length > 0 && (
        <div className="space-y-5 bg-indigo-50/30 p-6 rounded-3xl border border-indigo-50">
          <div className="flex items-center space-x-3 border-b border-indigo-100 pb-3">
            <div className="p-2 bg-indigo-100 rounded-lg">
              <Cpu className="w-6 h-6 text-indigo-700" />
            </div>
            <h3 className="text-2xl font-bold text-gray-900">Embedded Engineers</h3>
            <span className="bg-indigo-100 text-indigo-700 text-xs font-bold px-3 py-1 rounded-full shadow-sm">
